...
route.get("/profile/liked/:userId" , async (req,res) => {
    try{
        const likedProducts = await Product.find({ like : req.params.userId })
...
route.get("/profile/:username", async (req,res) => {
    try{
        const userProducts = await Product.find({ creator: req.params.username } || { owner: req.params.username })
